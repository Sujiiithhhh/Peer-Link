...
# echo "Cloning repository..."
 git clone https://github.com/Sujiiithhhh/Peer-Link.git
 cd Peer-Link
...
echo "Creating /etc/nginx/sites-available/Peer-Link..."
cat <<EOF | sudo tee /etc/nginx/sites-available/Peer-Link
server {
    listen 80;
...
sudo ln -sf /etc/nginx/sites-available/Peer-Link /etc/nginx/sites-enabled/Peer-Link
...
if [ $? -eq 0 ]; then
    ...
    echo "Nginx configuration test failed. Please check /etc/nginx/nginx.conf and /etc/nginx/sites-available/Peer-Link."
    exit 1
fi
...
pm2 start --name Peer-Link-backend java -- -cp "$CLASSPATH" p2p.App

# Start frontend with PM2
echo "Starting frontend with PM2..."
cd ui
pm2 start npm --name Peer-Link-frontend -- start
cd ..
...
echo "=== Setup Complete ==="
echo "Peer-Link is now running on your VPS!"
