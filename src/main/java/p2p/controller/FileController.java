...
import p2p.utils.EncryptionUtils;
import p2p.utils.QRCodeUtils;
...
public class FileController {
    ...
    public FileController(int port) throws IOException {
        ...
        server.createContext("/qr", new QRCodeHandler());
    ...
    private class QRCodeHandler implements HttpHandler {
        @Override
        public void handle(HttpExchange exchange) throws IOException {
            Headers headers = exchange.getResponseHeaders();
            headers.add("Access-Control-Allow-Origin", "*");
            ...
            try {
                ...
                QRCodeRequest request = objectMapper.readValue(requestBody, QRCodeRequest.class);
                
                // Generate QR code
                byte[] qrCodeBytes = QRCodeUtils.generateQRCode(request.data, request.width, request.height);
                
                // Return the QR code as PNG
                headers.add("Content-Type", "image/png");
                exchange.sendResponseHeaders(200, qrCodeBytes.length);
                try (OutputStream os = exchange.getResponseBody()) {
    ...
    private class UploadHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            try {
                ...
                boolean isEncrypted = filename.endsWith(".encrypted");
                String originalFilename = isEncrypted ? filename.replace(".encrypted", "") : filename;
                ...
                response.inviteCode = EncryptionUtils.generateInviteCode();
