...
import { EncryptionUtils, EncryptedData } from '@/utils/encryption';
...
export default function Home() {
  ...
  const handleFileUpload = async (file: File, encryptedData?: EncryptedData, key?: string) => {
  ...
  const handleDownload = async (port: number, key?: string) => {
    ...
    try {
      ...
      if (key && filename.endsWith('.encrypted')) {
        try {
          ...
          // Decrypt the file
          const decryptedArrayBuffer = EncryptionUtils.decryptFile(encryptedData, key);
          fileData = new Blob([decryptedArrayBuffer]);
          
          // Remove .encrypted extension
          filename = filename.replace('.encrypted', '');
        } catch (decryptError) {
          console.error('Error decrypting file:', decryptError);
          alert('Failed to decrypt file. Please check the encryption key.');
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <header className="text-center mb-12">
        ...
        <p className="text-xl text-gray-600">Secure P2P File Sharing with End-to-End Encryption</p>
      </header>
      ...
      <footer className="mt-12 text-center text-gray-500 text-sm">
        <p>PeerLink &copy; {new Date().getFullYear()} - Secure P2P File Sharing with End-to-End Encryption</p>
      </footer>
    </div>
  );
}
